...
public class EntityMara_Fusion extends EntityDBCNinjin {
    ...
    public void onUpdate() {
        ...
        // 使用配置文件中的时间间隔
        if (tickCounter % ModConfig.kiDrainIntervalFusion == 0) {
            // 使用配置文件中的范围
            List<EntityPlayer> playersInRange = getPlayersInRange(ModConfig.kiDrainRangeFusion);
            for (EntityPlayer player : playersInRange) {
                if (player != null) { // 检查玩家是否为 null
                    // 使用配置文件中的扣除量
                    int drainedKi = reducePlayerKi(player, ModConfig.kiDrainAmountFusion);
                    if (drainedKi > 0) {
    ...
    private void enhanceEntityWithKi(double ki) {
        ...
        double newMaxHealth = Math.floor(currentMaxHealth + ki * parseRatio(ModConfig.kiToMaxHealthRatioFusion, 1.0));
        double newAttackDamage = Math.floor(currentAttackDamage + ki * parseRatio(ModConfig.kiToMaxHealthRatioFusion, 1.0));
