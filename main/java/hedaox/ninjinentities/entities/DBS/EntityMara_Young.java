...
public class EntityMara_Young extends EntityDBCNinjin {
    ...
    public void onUpdate() {
        ...
        // 使用配置文件中的时间间隔
        if (tickCounter % ModConfig.kiDrainIntervalYoung == 0) {
            // 使用配置文件中的范围
            List<EntityPlayer> playersInRange = getPlayersInRange(ModConfig.kiDrainRangeYoung);
            for (EntityPlayer player : playersInRange) {
                if (player != null) { // 检查玩家是否为 null
                    // 使用配置文件中的扣除量
                    int drainedKi = reducePlayerKi(player, ModConfig.kiDrainAmountYoung);
                    if (drainedKi > 0) {
    ...
    private void healFromKi() {
        ...
        // 计算最大回复量（1% 最大血量）
        double maxHealAmount = maxHealth * (ModConfig.kiToHealthRatioYoung / 100.0);
    ...
    private void enhanceEntityWithKi(double ki) {
        ...
        double newMaxHealth = Math.floor(currentMaxHealth + ki * parseRatio(ModConfig.kiToMaxHealthRatioYoung, 1.0));
        double newAttackDamage = Math.floor(currentAttackDamage + ki * parseRatio(ModConfig.kiToAttackRatioYoung, 1.0));
